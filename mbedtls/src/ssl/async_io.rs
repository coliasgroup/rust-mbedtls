    ...
    for<'c, 'cx> (&'c mut TaskContext<'cx>, &'c mut T): IoCallbackUnsafe<AsyncStream>,
{
    fn poll_read(
        ...
        let poll = poll_to_io_poll(self.poll_recv_inner(cx, buf.initialize_unfilled()));
...
#[cfg(all(feature = "std", feature = "async"))]
fn poll_to_io_poll<T>(poll: Poll<Result<T>>) -> Poll<IoResult<T>> {
